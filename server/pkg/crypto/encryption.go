...
const (
	// EncryptionStringVersion is the version prefix for ciphertext
	EncryptionStringVersion = "v2"
	// EncryptionBinaryMagic is the magic bytes for binary format
	EncryptionBinaryMagic = "AFENC2"
	// EncryptionSaltSize is the salt size in bytes
	EncryptionSaltSize = 16
	// EncryptionKeySize is the derived key size (256 bits)
	EncryptionKeySize = 32
	// EncryptionPBKDF2Rounds is the number of PBKDF2 iterations
	EncryptionPBKDF2Rounds = 600000
)

// EncryptionService provides encryption and decryption for sensitive values.
// It uses AES-256-GCM with PBKDF2 key derivation.
...
// deriveKey derives a key from the passphrase using PBKDF2.
...
// encryptRaw encrypts plaintext bytes and returns the raw ciphertext (salt + nonce + ciphertext).
...
// decryptRaw decrypts raw ciphertext and returns the plaintext.
